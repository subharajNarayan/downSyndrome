...
if(!function_exists('gavias_post_type_team')){
  ...
  // -- Dynamic Experiencia Team Metabox -- 
  add_action( 'add_meta_boxes', 'gaviasthemer_team_experiencia' );
  add_action( 'save_post', 'team_experiencia_save_postdata' );
  function gaviasthemer_team_experiencia() {
    add_meta_box(
        'kuncothemer_team_experiencia',
        __( 'Experiencia', 'gaviasframework' ),
        'team_experiencia_inner_custom_box',
        'gva_team');
  }

  function team_experiencia_inner_custom_box() {
      global $post;
      wp_nonce_field( plugin_basename( __FILE__ ), 'dynamic_experiencia_noncename' );
      ?>
      <div id="meta_inner">
      <?php

      $team_experiencia = get_post_meta($post->ID, 'team_experiencia', true);

      $c = 0;
      if ( ($team_experiencia) && count( $team_experiencia ) > 0 ) {
          foreach( $team_experiencia as $experiencia ) {
              if ( isset( $experiencia['label'] )) {
                  printf( '<p><input size="80" type="text" placeholder="Label" name="team_experiencia[%1$s][label]" value="%2$s" /><a class="button remove">%3$s</a></p>', $c, $skill['label'], __( 'Remove' ) );
                  $c = $c +1;
              }
          }
      }
      ...
      ?>
  <span id="team-experiencia-list"></span>
  <a class="add-experiencia-item"><?php _e('Add Experiencia'); ?></a>
  <script>
      var $ =jQuery.noConflict();
      $(document).ready(function() {
          var count = <?php echo $c; ?>;
          $(".add-experiencia-item").click(function() {
              count = count + 1;
              $('#team-experiencia-list').append('<p><input size="80" type="text" placeholder="Label" name="team_experiencia['+count+'][label]" value="" /> <a class="remove button">Remove</a></p>' );
              return false;
          });
          $(".remove").on('click', function() {
              $(this).parent().remove();
  ...
  function team_experiencia_save_postdata( $post_id ) {
     if ( defined( 'DOING_AUTOSAVE' ) && DOING_AUTOSAVE ) 
          return;
     if ( !isset( $_POST['dynamic_experiencia_noncename'] ) )
          return;

     if ( !wp_verify_nonce( $_POST['dynamic_experiencia_noncename'], plugin_basename( __FILE__ ) ) )
          return;
     $team_experiencia = $_POST['team_experiencia'];
     update_post_meta($post_id,'team_experiencia', $team_experiencia);
  }
